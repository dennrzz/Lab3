...
bool Map::isPathR(City *source, City *target) {
    ...
    if(source == target) {
        ...
        nextCity = getNextCity(source);
        while(!done) {
            done = isPathR(nextCity, target);
            if (!done) {
                nextCity = getNextCity(source);
            } else {
                cout << *source << endl;
...
bool Map::isPathS(City *source, City *target) {
    ...
    //clear markings on all cities
    unvisitAll();

    //add starting city to stack
    cityStack.push(source);

    //ensure starting city is marked as visited
    markVisited(source);

    City* topCity = cityStack.peek();

    //loop through cities until target is reached
    while (!cityStack.isEmpty() && (topCity != target)) {
        City* nextCity = getNextCity(topCity);

        //if city is empty, remove from stack
        if (nextCity->getName() != target->getName()) {
            cityStack.pop();
        } else {
        ...
        //print out source if target is found
        if (topCity == target) {
            cout << source << endl;
        }
    }
...
//
//    LinkedBag<std::string> routes;
//    LinkedStack<std::string> tempStack;
//
//    string topOfStack = routes.();
//
